...
export const SCHEMA_DEFINITIONS: ServiceSchemaDefinition[] = [
    ...
    },
    {
        recordType: "Oura",
        serviceDir: "oura",
        description: "Oura Ring daily health and sleep data",
        columns: {
            type: { type: "VARCHAR", enum: ["Oura"], description: "Record type" },
            unixTimeMs: { type: "BIGINT", description: "Oura document timestamp as Unix milliseconds" },
            url: { type: "VARCHAR", nullable: true, description: "URL" },
            id: { type: "VARCHAR", description: "Oura document ID" },
            dataType: {
                type: "VARCHAR",
                enum: ["daily_activity", "daily_readiness", "daily_sleep", "sleep"],
                description: "Oura API data type",
            },
            day: { type: "VARCHAR", description: "Oura local day in YYYY-MM-DD format" },
            score: { type: "INTEGER", nullable: true, description: "Daily score from 1 to 100" },
            steps: { type: "INTEGER", nullable: true, description: "Daily step count" },
            activeCalories: { type: "INTEGER", nullable: true, description: "Active calories in kilocalories" },
            temperatureDeviation: { type: "DOUBLE", nullable: true, description: "Body temperature deviation in degrees Celsius" },
            bedtimeStart: { type: "VARCHAR", nullable: true, description: "Sleep start time in ISO 8601 format" },
            bedtimeEnd: { type: "VARCHAR", nullable: true, description: "Sleep end time in ISO 8601 format" },
            durationSeconds: { type: "INTEGER", nullable: true, description: "Total sleep duration in seconds" },
            averageHeartRate: { type: "DOUBLE", nullable: true, description: "Average sleeping heart rate in beats per minute" },
            averageHrv: { type: "INTEGER", nullable: true, description: "Average sleeping heart rate variability in milliseconds" },
            sleepType: { type: "VARCHAR", nullable: true, description: "Oura sleep period type" },
            rawData: { type: "VARCHAR", description: "Complete Oura API document as a JSON string" },
        },
    },
    {
